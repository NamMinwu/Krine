...
class FlowServiceTest {
    ...
    void 반론_답변과_종결_처리() {
        Long id = decisions.createDraft("고민").getId();
        ChallengeResult c = flow.challenge(id);
        assertThat(flow.answerChallenge(c.challengeId(), "업무 중 충전이 반복돼서요").reflectBack())
                .contains("맞나요");
        var response = flow.resolveChallenge(c.challengeId(), "DEFERRED");
        assertThat(response.challenges().get(0).resolution()).isEqualTo("DEFERRED");
    }
}
